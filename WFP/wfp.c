...
VOID UnInitWfp()
{
	if (EngineHandle != NULL)
	{
		if (filterid != 0)
		{
			...
			FwpmCalloutDeleteById(EngineHandle, &WFP_SAMPLE_SUB_LAYER_GUID);
		}
...
// 가장중요한 필터
VOID FilterCallback(FWPS_INCOMING_VALUE* Values, FWPS_INCOMING_METADATA_VALUES0 MetaData, PVOID* layerdata, PVOID context, FWPS_FILTER* filter, UINT64 flowcontext, FWPS_CLASSIFY_OUT* classify)
{
...
NTSTATUS WfpAddCallout()
{
	...
	callout.calloutId = WFP_SAMPLE_ESTABLEISHED_CALLOUT_V4_GUID;
