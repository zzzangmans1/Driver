...
특정 IRQL 루틴은 동일한 IRQL 레벨값을 사용하는 작업을 선점하지 못한다는 사실을 바탕으로, 드라이버 개발자는 방어의 목적으로 IRQL 값을 스스로 조정하기도 한다.

이때 사용하는 함수가 KeRaiseIrql(), KeLowerIrql() 함수다.
드라이버 개발자는 이 함수를 사용해서 특정 IRQL 수준으로 현재의 작업 우선순위를 조정할 수 있다.
이렇게 함으로써 관련된 다른 작업으로부터 CPU가 선점되는 것을 방어할 수 있다.

``` C
// WDK가 제공하는 KeRaiseIrql(), KeLowerIrql() 프로토 타입
VOID KeRaiseIrql(
  IN KIRQL NewIrql, // 올리고 싶은 새로운 IRQL 값
  OUT PKIRQL OldIrql // 현재 사용 중이었던 IRQL 값
  );

VOID KeLowerIrql(
  IN KIRQL OldIrql // 환원하고자 하는 IRQL 값
  );
```

독자들이 꼭 기억해야 할 점은 IRQL **DISPATCH_LEVEL** 수준 이상의 작업을 진행하는 상태에서는 절대로 스케쥴링 혹은 페이징이 발생하지 않도록 조심해야 한다는 것이다.
이와 같은 작업은 현재 스레드를 블로킹 시키게 되는데, 앞에서 언급했던 대로 인터럽트는 커널이 수행하는 작업을 선점하는 작업이며 이렇게 선점되는 커널 작업에는 스케쥴링과 관련된 작업도 포함될 수 있다.
그러므로 운영체제는 인터럽트를 처리하는 루틴은 절대로 본인이 직접 스케줄링을 일으키는 작업을 하지 못하도록 금지하고 있다.
페이징 작업도 역시 일종의 스케쥴링을 일으키게 되므로 금지된다.

### 2.2.3.4 핸들과 프로세스

핸들이라는 용어는 권한을 나타내는 용도로 사용된다.
핸들값이 의미를 가지는 영역은 현재 프로세스 영역이다.
윈도우는 주요 객체들에 대해, 이들을 커널 레벨에 보관하면서 외부로부터 이를 접근하는 행위에 대한 참조 관리와 보호 관리를 수행한다.
동일한 대상이라고 하더라도, 접근하려는 행위의 조건과 접근하는 스레드에 따라서 저마다 각각의 접근 권한이 할당된다.
이와 같이 핸들은 당시 프로세스에만 연관을 가지기 때문에, 당시 프로세스와 다른 프로세스가 현재 프로세스인 경우이 값은 아무 의미 없는 핸들값이 된다.
